...
//   }
  return (
    ...
    <div>
      <CardHeader sx={{ padding: 0.75 }}
        avatar={<ProfileImage type="large" src=""></ProfileImage>}
        title={<ButtonStyle size={"medium"} variant={"primary"}>테스트 중</ButtonStyle>}>
      </CardHeader>
      <div style={{ marginLeft: 20 }}>
        <TextStyle size="large">{props.username}    </TextStyle>
        <TextStyle size="medium">{props.usertag}</TextStyle>
        <br></br>
        <TextStyle size="medium">팔로워 : {props.follower} | 팔로잉 : {props.following}</TextStyle>
        <br></br>
