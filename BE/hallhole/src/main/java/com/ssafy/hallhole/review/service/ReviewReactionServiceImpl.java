...
public class ReviewReactionServiceImpl implements ReviewReactionService {
    ...
    @Override
    public List<ReviewReaction> getMemberReactionList(int start, int size, String memberTag) throws NotFoundException {
        Member m = memberRepository.findByIdTag(memberTag);
        if(m==null || m.isOut()){
            throw new NotFoundException(" 유효한 사용자가 아닙니다.");
        }

        return rrRepository.findAllByMemberId(start, size, m.getId());
    }
